...
const token = () => jwt.sign(user, settings.secret, { audience: 'scele-tracker', issuer: settings.origin });
describe('authentication boundary', () => {
  it('protects both list and details without syncing', async () => {
    const get = vi.fn(); const app = createApp({ get });
  ...
  it('accepts valid SSO sessions and does not cache private responses', async () => {
    const get = vi.fn().mockResolvedValue({ activities: [], sources: [], configured: false });
    const response = await request(createApp({ get })).get('/api/activities').set('Cookie', `scele_session=${token()}`);
  ...
  it('rejects forged and wrong-audience JWTs', () => {
    ...
    expect(sessionUser(jwt.sign(user, settings.secret, { audience: 'another-app' }))).toBeNull();
  });
  it('rejects unbound CAS callbacks without contacting CAS', async () => {
    const fetch = vi.fn(); vi.stubGlobal('fetch', fetch);
    const response = await request(createApp()).get('/api/auth/cas/callback?ticket=ST-test&state=bad');
    expect(response.headers.location).toContain('login_expired'); expect(fetch).not.toHaveBeenCalled();
  });
  it('requires same-origin logout', async () => {
    expect((await request(createApp()).post('/api/auth/logout').set('Origin', 'https://evil.example')).status).toBe(403);
    expect((await request(createApp()).post('/api/auth/logout').set('Origin', settings.origin)).status).toBe(200);
  });
  it('parses CAS namespaces and rejects failures or entities', () => {
    expect(parseIdentity('<cas:serviceResponse><cas:authenticationSuccess><cas:user>student</cas:user><cas:attributes><cas:nama>Student</cas:nama></cas:attributes></cas:authenticationSuccess></cas:serviceResponse>')).toEqual(user);
    expect(() => parseIdentity('<serviceResponse><authenticationFailure>no</authenticationFailure></serviceResponse>')).toThrow();
...
describe('accounts and metadata', () => {
  ...
  it('strips active markup and normalizes missing timestamps', () => {
    expect(plainText('<p>Hello</p><script>alert(1)</script><img src=x onerror=evil()>')).toBe('Hello');
    expect(timestamp(0)).toBeNull(); expect(timestamp(-1)).toBeNull(); expect(timestamp(Infinity)).toBeNull(); expect(timestamp(123)).toBe(123);
  });
  it('merges calendar open and due events but preserves account-specific variants', () => {
    const items: Activity[] = [];
    const event = { modulename: 'quiz', instance: 42, activityname: 'Quiz 1', name: 'Quiz closes', eventtype: 'close', timestart: 200, course: { id: 2, fullname: 'Logic' } };
    mergeCalendar(items, [event, { ...event, eventtype: 'open', timestart: 100 }], { id: 'one', mode: 'token' });
    mergeCalendar(items, [event], { id: 'two', mode: 'token' });
    expect(items).toHaveLength(2); expect(items[0].opensAt).toBe(100); expect(items[0].dueAt).toBe(200);
