...
export function parseAccounts(value: unknown): Account[] {
  ...
  return value.accounts.map((account: Account) => {
    if (
      ...
      !/^[a-z0-9-]{1,40}$/.test(account.id) ||
      ids.has(account.id)
    )
      throw new Error('Account IDs must be unique public aliases');
    ids.add(account.id);
    if (account.mode === 'session') {
      if (
        ...
        throw new Error('Session credentials required');
      return {
        id: account.id,
        mode: account.mode,
    ...
    if (account.mode === 'token' && typeof account.token === 'string' && account.token)
      return { id: account.id, mode: account.mode, token: account.token };
