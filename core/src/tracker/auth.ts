...
const options = { httpOnly: true, secure: settings.secure, sameSite: 'lax' as const, path: '/' };
export function sessionUser(token: unknown) {
  if (typeof token !== 'string') return null;
  try {
    const value = jwt.verify(token, settings.secret, { algorithms: ['HS256'], audience: 'scele-tracker', issuer: settings.origin });
    if (typeof value === 'string' || typeof value.username !== 'string' || typeof value.fullname !== 'string') return null;
    return { username: value.username, fullname: value.fullname };
  } catch { return null; }
}
export const requireSession: RequestHandler = (req, res, next) => {
  if (!sessionUser(req.cookies[cookie])) { res.status(401).json({ error: 'Sign in with UI SSO to continue.' }); return; }
  next();
};
export const sameOrigin: RequestHandler = (req, res, next) => {
  if (req.get('origin') !== settings.origin) { res.status(403).json({ error: 'Invalid request origin.' }); return; }
  next();
};
export function parseIdentity(xml: string) {
  if (xml.length > 100_000 || /<!DOCTYPE|<!ENTITY/i.test(xml)) throw new Error('Invalid CAS response');
  ...
  if (!success || typeof success.user !== 'string' || !success.user.trim()) throw new Error('CAS authentication failed');
...
auth.get('/cas/callback', async (req, res) => {
  ...
  if (typeof state !== 'string' || !/^[a-f0-9]{64}$/.test(state) || typeof expected !== 'string' || expected.length !== state.length || !timingSafeEqual(Buffer.from(state), Buffer.from(expected)) || typeof ticket !== 'string' || ticket.length > 2048) {
    res.redirect('/?error=login_expired'); return;
  }
  try {
    const service = `${settings.origin}/api/auth/cas/callback?state=${state}`;
    const response = await fetch(`${settings.cas}/serviceValidate?${new URLSearchParams({ service, ticket })}`, { signal: AbortSignal.timeout(15_000), redirect: 'error' });
    if (!response.ok) throw new Error('CAS unavailable');
    const user = parseIdentity(await response.text());
    const token = jwt.sign(user, settings.secret, { algorithm: 'HS256', expiresIn: '8h', audience: 'scele-tracker', issuer: settings.origin });
