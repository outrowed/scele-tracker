...
export function createTracker() {
  let snapshot: { activities: Activity[]; sources: SourceStatus[]; configured: boolean; checkedAt: string | null } = { activities: [], sources: [], configured: false, checkedAt: null };
  ...
  async function sync() {
    ...
    for (const account of accounts) {
      try {
        const result = await (account.mode === 'session' ? syncSession(account) : syncToken(account));
        activities.push(...result.activities);
        sources.push({ id: account.id, state: result.complete ? 'ok' : 'partial', updatedAt: new Date().toISOString() });
      } catch {
    ...
    snapshot = { activities, sources, configured: accounts.length > 0, checkedAt: new Date().toISOString() };
    checked = Date.now();
    return snapshot;
  }
  return {
