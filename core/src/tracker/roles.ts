...
export function parseUsers(value: unknown): Map<string, Role> {
  if (
    ...
    throw new Error('Invalid users configuration');
  ...
  for (const entry of value.users) {
    if (
      ...
      users.has(entry.username) ||
      !['admin', 'user'].includes(entry.role)
    )
...
// Resolve roles from the file on every request, not JWT claims, so demotion is immediate.
export async function roleFor(username: string): Promise<Role> {
  try {
    const path =
      process.env.USERS_FILE ||
      resolve(import.meta.dirname, '../../../config/users.json');
